...
/// Breeding tab shell: litter list entry until full I3 wizard lands.
class BreedingHubPage extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    return AnimatedBuilder(
      ...
      builder: (context, _) {
        final snapshot = controller.snapshotState.data;
        final litters = snapshot?.litters ?? const <I2Litter>[];
        final active = litters
            .where((l) => HomeOverviewMetrics._isActiveLitter(l))
            .length;
        return ListView(
          padding: const EdgeInsets.fromLTRB(16, 20, 16, 32),
          children: [
            ...
            const SizedBox(height: 4),
            const Text(
              '计划动作向导将在后续迭代接入；当前可管理窝次。',
              style: TextStyle(color: Color(0xff6c7774)),
            ),
            const SizedBox(height: 16),
            Card(
              child: ListTile(
                ...
                subtitle: Text(
                  litters.isEmpty ? '暂无窝次' : '共 ${litters.length} 窝 · 活跃 $active',
                ),
                trailing: const Icon(Icons.chevron_right),
                onTap: onOpenLitters,
              ),
            ...
            const Card(
              child: ListTile(
                ...
                title: Text('繁育计划向导'),
                subtitle: Text('配对 → 分笼 → 孕期 → 产仔（T-P0-02）'),
                enabled: false,
              ),
            ),
          ],
