...
- **M1–M4 前端业务页已写入**：真实 API、写操作、权限/会话、错误态、空态、离线只读快照和数据中心入口均已接入；笼舍检索/结束入住/设施编辑/真实个体选择、个体完整档案字段与核心表型目录、个体头像媒体上传/移除、繁育结束配对分笼、AI 任务草案预填、遗传目标交配与反馈闭环、公开个体资料管理、记账分类管理、合同/回执模板管理与 CRM 关联选择也已接入。
