...
export default function AccountingCategoriesPage() {
  ...
  return <View style={{ height: '100vh', display: 'flex', flexDirection: 'column', backgroundColor: palette.systemBackground }}><NavBar title="记账分类" back /><ScrollView scrollY style={{ flex: 1 }}><SectionList>
    ...
    <Section header="已有分类" footer={`共 ${categories.length} 个`}>
      {categories.length ? categories.map((item) => <Cell key={item.id} title={item.name} subtitle={item.entryType === 'income' ? '收入' : '支出'} value={<Tag tone={item.entryType === 'income' ? 'success' : 'warning'}>{item.entryType === 'income' ? '收入' : '支出'}</Tag>} />) : <Cell title="尚无分类" subtitle="新建后可在收支记录中选择" />}
