...
export default function AnimalDetailPage() {
  ...
  return (
    <View style={{ height: '100vh', display: 'flex', flexDirection: 'column', backgroundColor: crayon.paper, backgroundImage: paperGrain }}>
      <NavBar title={animal?.name || animal?.internalCode || '个体档案'} back right={<Tag tone="accent">档案</Tag>} />
        ...
        {animal ? (
          <SectionList>
            <Section header="基本信息" footer={message || '记录会进入后端审计链'}>
              {avatarUrl ? <Image src={avatarUrl} mode="aspectFill" style={{ width: '96px', height: '96px', borderRadius: '48px', margin: '12px auto', display: 'block' }} /> : <Cell title="暂无头像" />}
              <Cell title={animal.name || animal.internalCode} subtitle={`${animal.internalCode} · ${animal.varietyCode || '品系未记录'}`} value={<Tag tone="success">{animal.lifecycleStatus}</Tag>} />
              <Cell title="当前笼舍" value={animal.currentEnclosureId || '未分配'} />
              <Cell title="繁育状态" value={animal.breedingStatus || '未记录'} />
            </Section>
            <Section header="头像">
            ...
            <Section header="档案编辑">
              <FormRow label="内部编号"><Input value={animalInternalCode} placeholder="内部编号" onInput={(event) => setAnimalInternalCode(event.detail.value)} /></FormRow>
              <FormRow label="名称" divider><Input value={animalName} placeholder="可选" onInput={(event) => setAnimalName(event.detail.value)} /></FormRow>
              <FormRow label="性别" divider><Picker mode="selector" range={['待定', '公', '母']} value={Math.max(0, ['unknown', 'male', 'female'].indexOf(animalSex))} onChange={(event) => setAnimalSex(['unknown', 'male', 'female'][Number(event.detail.value)] || 'unknown')}><Input value={animalSex === 'male' ? '公' : animalSex === 'female' ? '母' : '待定'} disabled /></Picker></FormRow>
              {seriesOptions.length ? <><FormRow label="表型系列" divider><Picker mode="selector" range={seriesOptions.map((item) => item.name)} value={Math.max(0, seriesOptions.findIndex((item) => item.code === seriesCode))} onChange={(event) => { const selected = seriesOptions[Number(event.detail.value)]; setSeriesCode(selected?.code || ''); setPhenotypeLabel(selected?.phenotypes[0] || ''); setAnimalVarietyCode(encodePhenotype(selected?.code || '', selected?.phenotypes[0] || '')) }}><Input value={selectedSeries?.name || '选择系列'} disabled /></Picker></FormRow><FormRow label="表型" divider><Picker mode="selector" range={phenotypeOptions} value={phenotypeIndex} onChange={(event) => { const label = phenotypeOptions[Number(event.detail.value)] || ''; setPhenotypeLabel(label); setAnimalVarietyCode(encodePhenotype(seriesCode, label)) }}><Input value={phenotypeLabel || '选择表型'} disabled /></Picker></FormRow></> : null}
              <FormRow label="品系代码" divider><Input value={animalVarietyCode} placeholder={seriesOptions.length ? '可选，默认使用系列|表型' : '可选'} onInput={(event) => setAnimalVarietyCode(event.detail.value)} /></FormRow>
              <FormRow label="出生日期" divider><Picker mode="date" value={animalBirthDate} onChange={(event) => setAnimalBirthDate(event.detail.value)}><Input value={animalBirthDate || '未填写'} disabled /></Picker></FormRow>
              <FormRow label="备注" divider><Textarea value={animalNotes} maxlength={1000} placeholder="可选" onInput={(event) => setAnimalNotes(event.detail.value)} /></FormRow>
            ...
            <Section header="体重快录">
              <FormRow label="克数"><Input type="digit" placeholder="例如 128.5" value={weight} onInput={(event) => setWeight(event.detail.value)} /></FormRow>
              <FormRow label="备注" divider><Input placeholder="可选" value={note} onInput={(event) => setNote(event.detail.value)} /></FormRow>
            ...
            <Section header="最近体重" footer={`共 ${weights.length} 条`}>
              {weights.length ? weights.slice(0, 10).map((item) => <Cell key={item.id} title={`${item.weightG ?? item.weight_g ?? '-'} g`} subtitle={String(item.recordedAt || item.recorded_at || '')} value={<Tag>{item.source || 'manual'}</Tag>} />) : <Cell title="暂无体重记录" />}
            </Section>
            <Section header="健康记录" footer={`共 ${health.length} 条`}>
              {health.length ? health.slice(0, 10).map((item) => <Cell key={item.id} title={item.type} subtitle={item.notes || String(item.observedAt || '')} value={<Tag tone={item.severity === 'high' || item.severity === 'critical' ? 'danger' : 'success'}>{item.severity || 'info'}</Tag>} />) : <Cell title="暂无健康记录" />}
            </Section>
