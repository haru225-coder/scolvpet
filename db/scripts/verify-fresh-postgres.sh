...
# Baseline after migrations 0000–0044 (I1–I6 + CRM/合同/公开主页/预订/Assistant
...
# subscription event delivery queue.
...
[[ "$MIGRATIONS" == "45" ]] || { printf 'migration count mismatch: %s\n' "$MIGRATIONS" >&2; exit 1; }
[[ "$LAST_MIGRATION" == "0044_breeder_wechat_subscription_events.sql" ]] || { printf 'last migration mismatch: %s\n' "$LAST_MIGRATION" >&2; exit 1; }
