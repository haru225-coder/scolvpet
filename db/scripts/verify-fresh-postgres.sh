...
# Baseline after migrations 0000–0041 (I1–I6 + CRM/合同/公开主页/预订/Assistant
...
[[ "$MIGRATIONS" == "42" ]] || { printf 'migration count mismatch: %s\n' "$MIGRATIONS" >&2; exit 1; }
[[ "$LAST_MIGRATION" == "0041_composite_fk_set_null_columns.sql" ]] || { printf 'last migration mismatch: %s\n' "$LAST_MIGRATION" >&2; exit 1; }
