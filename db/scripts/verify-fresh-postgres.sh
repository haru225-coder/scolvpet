...
# Baseline after migrations 0000–0045 (I1–I6 + CRM/合同/公开主页/预订/Assistant
# and the P0 security persistence tables, including B-side WeChat identity,
# subscription grants, and subscription event delivery queue).
...
# 0040–0044 add customer/B-side WeChat identity, subscription grants, and the
# subscription event delivery queue; 0045 enforces one active WeChat binding
# per customer phone.
[[ "$TABLES" == "78" ]] || { printf 'table count mismatch: %s\n' "$TABLES" >&2; exit 1; }
...
[[ "$MIGRATIONS" == "46" ]] || { printf 'migration count mismatch: %s\n' "$MIGRATIONS" >&2; exit 1; }
[[ "$LAST_MIGRATION" == "0045_customer_wechat_phone_unique.sql" ]] || { printf 'last migration mismatch: %s\n' "$LAST_MIGRATION" >&2; exit 1; }
